"""g2p_clean.py — E59: label-independent G2P references — the
clean instrument after E55's admission hold (seq 271/273).
...
def main():
    ...
    arms = [("none", "G2P-NONE"), ("graph", "G2P-GRAPHEMIC"),
            ("rand", "G2P-RANDOM-ANCHOR"), ("oracle", "G2P-ORACLE-RIME")]
    ...
    none_m, graph_m = meanpos("G2P-NONE"), meanpos("G2P-GRAPHEMIC")
    rand_m, oracle_m = meanpos("G2P-RANDOM-ANCHOR"), \
        meanpos("G2P-ORACLE-RIME")
    p1 = abs(none_m - freq_pos) <= 0.05
    p2 = (graph_m - rand_m) >= 0.08
    p3 = (oracle_m >= graph_m > rand_m
          and abs(rand_m - none_m) <= 0.05)
    print("\n=== VERDICTS (pre-registered seq 274) ===")
    print(f"  P1 (NONE ~= FREQ-MOD8): "
          f"{'CONFIRMED' if p1 else 'FALSIFIED'} "
          f"({none_m} vs {freq_pos})")
    print(f"  P2 (GRAPHEMIC beats RANDOM by >=8pp): "
          f"{'CONFIRMED' if p2 else 'FALSIFIED'} "
          f"({graph_m} vs {rand_m})")
    print(f"  P3 (ceiling ordering): "
          f"{'CONFIRMED' if p3 else 'FALSIFIED'} "
          f"(oracle {oracle_m} >= graph {graph_m} > rand {rand_m}"
          f" ~= none {none_m})")
    log("verdicts", P1_floor=p1, P2_graphemic_beats_random=p2,
        P3_ordering=p3, none=none_m, graph=graph_m, rand=rand_m,
        oracle=oracle_m, freq=freq_pos)
    save()
